...
```
alias dockerps='docker container ls --format "table {{.ID}}\t{{.Names}}\t{{.Command}}\t{{.RunningFor}}\t{{.Status}}\t{{.Networks}}" | (sed -u 1q; sort -k2)'
```
...
# remove entry from reverseproxy nc.conf
# remove entry from instance list
# stop docker containers

pname=nc1
thispath=apps/nextcloud/standard
docker compose -p $pname -f $thispath/mariadb.yml -f $thispath/nextcloud.yml -f $thispath/redis.yml -f $thispath/appapi.yml  down
docker compose -p $pname -f $thispath/mariadb.yml -f $thispath/nextcloud.yml -f $thispath/redis.yml -f $thispath/appapi.yml up -d

pname=test
thispath=apps/nextcloud/standard
docker compose -p $pname -f $thispath/mariadb.yml -f $thispath/nextcloud.yml -f $thispath/redis.yml  down
docker compose -p $pname -f $thispath/mariadb.yml -f $thispath/nextcloud.yml -f $thispath/redis.yml  up -d
...
sudo nano /etc/hosts
# or
echo '172.17.0.1 nc6.local.mlh.ovh' | sudo tee -a /etc/hosts
```

## change the reverse proxy configuration

```
# change the config
nano apps/reverseproxy/conf/nc.conf

# apply the changes
bash apps/reverseproxy/update.sh
...
thispath=apps/nextcloud/standard
docker compose -p 'test' -f $thispath/mariadb.yml -f $thispath/nextcloud.yml -f $thispath/redis.yml up -d

# to stop the new instance
docker compose -p 'test' -f $thispath/mariadb.yml -f $thispath/nextcloud.yml -f $thispath/redis.yml  down
```

## access logs, data and/or config.php

```
pname=ncgss1
sudo nano /var/lib/docker/volumes/$pname\_nextcloud/_data/config/config.php
sudo tail /var/lib/docker/volumes/$pname\_nextcloud/_data/data/nextcloud.log
sudo ls /var/lib/docker/volumes/$pname\_db/_data/
```

## tweak some stuff

```
cat apps/nextcloud/instancelist.txt | while read instancename;
do
    echo $instancename-nextcloud-1
...
docker exec -u 33 $instancename-nextcloud-1 php occ config:app:set --value=0 user_oidc allow_multiple_user_backends
docker exec -u 33 $instancename-nextcloud-1 php occ config:app:set --value="email" user_oidc provider-1-mappingEmail
docker exec -u 33 $instancename-nextcloud-1 php occ config:app:set --value="preferred_username" user_oidc provider-1-mappingUid
docker exec -u 33 $instancename-nextcloud-1 php occ config:app:set --value=0 user_oidc provider-1-uniqueUid


### enable richdocuments
...
do
    ...
    # docker exec -u 33 $instancename-nextcloud-1 php occ config:app:set richdocuments public_wopi_url --value='https://office.local.mlh.ovh'
    docker exec -u 33 $instancename-nextcloud-1 php occ config:app:set richdocuments wopi_url --value='https://office.local.mlh.ovh'
    docker exec -u 33 $instancename-nextcloud-1 php occ richdocuments:activate-config
done
```

### enable the witheboard

```
    #docker exec -u 33 $instancename-nextcloud-1 php occ app:enable whiteboard
    #docker exec -u 33 $instancename-nextcloud-1 php occ config:app:set whiteboard collabBackendUrl --value="http://whiteboard"
    #docker exec -u 33 $instancename-nextcloud-1 php occ config:app:set whiteboard jwt_secret_key --value="some-random-key"
```
...
do
    echo $instancename-nextcloud-1
    docker exec -u 33 $instancename-nextcloud-1 php occ config:system:set gs.trustedHosts 0 --value="*.local.mlh.ovh"
    docker exec -u 33 $instancename-nextcloud-1 php occ config:app:set richdocuments federation_use_trusted_domains --value="yes"
done
```
...
for i in $(seq 2 3);
do
    docker exec -it -u 33 nc$i-nextcloud-1 php cron.php
    docker exec -it -u 33 nc$i-nextcloud-1 php occ dav:sync-system-addressbook
    docker exec -it -u 33 nc$i-nextcloud-1 php occ federation:sync-addressbooks
done
```
...
# create life on the instance

## create users

```
for instancename in ncgss2;
do
    curl -X POST https://admin:admin@$instancename.local.mlh.ovh/ocs/v1.php/cloud/users -d userid="alice" -d password="alice" -H "OCS-APIRequest: true"
     curl -X PUT https://admin:admin@$instancename.local.mlh.ovh/ocs/v1.php/cloud/users/alice -d key="displayname" -d value="Alice" -H "OCS-APIRequest: true"
    curl -X POST https://admin:admin@$instancename.local.mlh.ovh/ocs/v1.php/cloud/users -d userid="bob" -d password="bob" -H "OCS-APIRequest: true"
     curl -X PUT https://admin:admin@$instancename.local.mlh.ovh/ocs/v1.php/cloud/users/bob -d key="displayname" -d value="Bob" -H "OCS-APIRequest: true"
    curl -X POST https://admin:admin@$instancename.local.mlh.ovh/ocs/v1.php/cloud/users -d userid="charlie" -d password="charlie" -H "OCS-APIRequest: true"
     curl -X PUT https://admin:admin@$instancename.local.mlh.ovh/ocs/v1.php/cloud/users/charlie -d key="displayname" -d value="Charlie" -H "OCS-APIRequest: true"
    curl -X POST https://admin:admin@$instancename.local.mlh.ovh/ocs/v1.php/cloud/users -d userid="david" -d password="david" -H "OCS-APIRequest: true"
     curl -X PUT https://admin:admin@$instancename.local.mlh.ovh/ocs/v1.php/cloud/users/david -d key="displayname" -d value="David" -H "OCS-APIRequest: true"
done
```
