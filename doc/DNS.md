...
# replace existing symlink of resolv.conf from the disabled systemd-resolved.service which points to ../run/systemd/resolve/stub-resolv.conf by the new from NetworkManager
...
`sudo cat /etc/resolv.conf` should show something similar to:
